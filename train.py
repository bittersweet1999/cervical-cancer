...
##            
        ...
        if args.cont:
            ...
            for i in pos_error_list_label:
                pos_error_dict[i] = pos_error_dict.get(i,0)+1
            pos_cont_lines = [str(k).strip() + ': ' + str(v) + '\n' for k,v in pos_error_dict.items()]    

            with open(args.logdir,'a') as f:
                f.write('**'*20 + f'train epoch {epoch}' + '**'*20 +'\n')
                #f.writelines(train_error_list)
                #f.write(f'-------cont------------\n')
                f.writelines(train_cont_lines)
                f.write(f'------------eval------------\n')
                f.writelines(pos_cont_lines)
