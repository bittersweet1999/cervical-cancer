...
from utils.utils import  train_one_epoch_multi_longtail_weight,evaluate_multi_longtail_weight,train_one_epoch_multi_moe_dis,evaluate_multi_moe_dis
...
def main(args):
   ...
   # model.heads[1-args.head_idx].requires_grad_(False)
    ...
    model = model.to(device) # 创建模型实例
    model.load_state_dict(torch.load(args.weights)) # 加载模型状态
    ...
    for epoch in range(args.epochs):
        ...
        print('dasdsad',epoch,len(train_loader))
        # if epoch == 0:
        #     for x,y in train_loader:
        #         print(x.cuda(args.local_rank).size(),'sads')
        print(epoch,args.local_rank)
        # train
# #
...
#                                                     )
#         elif args.backbone == 'vit_moe':
#                 train_loss, train_accs, train_error_list, train_senss, train_specs = train_one_epoch_multi_moe_dis(model=model,
#                                             optimizer=optimizer,
#                                             data_loader=train_loader,
...
##            
        if args.backbone == 'vit_res':
            ...
            val_loss, val_accs, _, val_senss, val_specs = evaluate_multi_moe_dis(model=model,
                            data_loader=val_loader,
                            local_rank=args.local_rank,
                            epoch=epoch,
                            multi_tasks=args.multi_tasks,
        ...
        if args.local_rank == 0:
            ...
            else:torch.save(model.module.state_dict(), args.where+"/model-{}-{}.pth".format(args.head_idx,epoch))
...
if __name__ == '__main__':
    ...
    opt = get_args()
    print(opt)
    if opt.local_rank == 0 and not os.path.exists(opt.where) :
