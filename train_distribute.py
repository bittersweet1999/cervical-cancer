...
# #
        if args.backbone == 'vit_res':
            train_loss, train_accs, train_error_list = train_one_epoch_multi_longtail_weight(model=model,
                ...
                train_loss, train_accs, train_error_list, train_senss, train_specs = train_one_epoch_multi_moe_dis2(model=model,
                                            optimizer=optimizer,
                                            data_loader=train_loader,
...
##            
        if args.backbone == 'vit_res':
            ...
            val_loss, val_accs, val_error_list, val_senss, val_specs = evaluate_multi_moe_dis(model=model,
                            data_loader=val_loader,
                            local_rank=args.local_rank,
                            epoch=epoch,
                            multi_tasks=args.multi_tasks,
