...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        #x =torch.cat([x[:,0:10,:].mean(axis=1,keepdim=True),x[:,10:20,:].mean(axis=1,keepdim=True)],dim=1)
        x = self.blocks2(x)
        x = self.norm(x)
...
class VisionTransformerMulti(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.0, qkv_bias=True,
                 qk_scale=None, representation_size=None, distilled=False, drop_ratio=0.,
                 attn_drop_ratio=0., drop_path_ratio=0., embed_layer=PatchEmbed, norm_layer=None,
                 act_layer=None,multi_tasks=1,head_idx=None):
        """
        Args:
            img_size (int, tuple): input image size
            patch_size (int, tuple): patch size
            ...
            norm_layer: (nn.Module): normalization layer
        """
        super(VisionTransformerMulti, self).__init__()
        ...
        self.task_tokens = nn.Parameter(torch.zeros(1, multi_tasks, embed_dim))

        self.heads = [ nn.Linear(self.num_features, num_classes[i]).cuda() for i in range(self.multi_tasks) ]
    ...
    def forward_features(self, x):
        ...
        cls_tokens=torch.cat((cls_token,cls_token2),dim=1)
        ...
        #print((x[:,0] == x[:,1]).sum() / x[0].numel(),x.size())
        # x = x.mean(axis=1,keepdim=False)

        # print((x[:,0]==x[:,1]).sum()/x[:,0].numel())
    ...
    def forward(self, x):
        # print(x.shape)
        x = self.forward_features(x)
        # print(x.size(),x[:,0].size(),(x[:,0]==x[:,1]).sum()/x[:,0].numel())
...
class VisionTransformerMulti_LongTail(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.0, qkv_bias=True,
                 qk_scale=None, representation_size=None, distilled=False, drop_ratio=0.,
                 attn_drop_ratio=0., drop_path_ratio=0., embed_layer=PatchEmbed, norm_layer=None,
        ...
        Args:
            ...
            norm_layer: (nn.Module): normalization layer
        """
        super(VisionTransformerMulti_LongTail, self).__init__()
        if isinstance(num_classes,int):
            num_classes = [num_classes] * multi_tasks
...
def vit_base_patch16_224_in21k_multi(embed_dim=512, num_classes: int = 21843, has_logits: bool = True, multy_tasks=2,head_idx=None):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    weights ported from official Google JAX impl:
    https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-vitjx/jx_vit_base_patch16_224_in21k-e5005f0a.pth
    """
    model = VisionTransformerMulti(img_size=224,
                              patch_size=16,
                              ...
                              representation_size=512 if has_logits else None,
                              num_classes=num_classes,
                              multi_tasks=multy_tasks,
                              head_idx=head_idx
                                   )
    return model

def vit_base_patch16_224_in21k_multi_long_tail(embed_dim=512, num_classes: int = 21843, has_logits: bool = True, multy_tasks=2,head_idx=None,long_tail=False,alpha=1):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    weights ported from official Google JAX impl:
    https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-vitjx/jx_vit_base_patch16_224_in21k-e5005f0a.pth
    """
    model = VisionTransformerMulti_LongTail(img_size=224,
                              patch_size=16,
                              embed_dim=embed_dim,#768
                              depth=4,
                              num_heads=2,#16
...
if __name__=='__main__':
    ...
    model2 = VisionTransformer(img_size=224,
                              patch_size=16,
                              embed_dim=512,
                              depth=4,
                              num_heads=2,
