...
class VitResMoE(nn.Module):
    def __init__(self,embed_dim=512,num_classes=args.num_classes, has_logits=False,multi_tasks=args.multi_tasks,long_tail=args.long_tails,alpha=args.alpha,depth=args.depth,moe_top_k=args.moe_top_k,moe_gate_dim=args.gate_dim,moe_experts=args.moe_experts):
        super().__init__()
        resnet = models.__dict__[args.arch](pretrained=True)
        resnet = torch.nn.Sequential(*(list(resnet.children())[:-1]))
        self.resnet = resnet
        ...
        if args.res_weights:
            print('load resnet weights -------------------------------------------------',self.resnet.load_state_dict(torch.load(args.res_weights)))
        ...
        self.model = vit_multi_long_tail_MoE(embed_dim=embed_dim, num_classes= num_classes, has_logits = has_logits, multi_tasks=multi_tasks,depth=depth,long_tail=long_tail,alpha=alpha,num_heads=16,\
                            ...
                            moe_gate_type="noisy_vmoe", vmoe_noisy_std=0.1, gate_task_specific_dim=-1,multi_gate=True,regu_experts_fromtask = False, 
